...
if __name__ == '__main__':
    ...
    parser.add_argument('--end_epochs', default = 50, type=int, help='Number of training epoch')
    parser.add_argument('--parallel',default = False, type=bool, help='Whether use parallel to train')
    parser.add_argument('--name',default = 'resnet18pre_simLoss_085_3', type=str, help='Name of the saved file')
    args = parser.parse_args()

    jcn_matrix = np.load('../data/jcn_similarity_matrix.npy')
    wp_matrix = np.load('../data/wu_palmer_similarity_matrix.npy')
    target_matrix = torch.from_numpy(generate_target_sim(wp_matrix, topk=5, gap=0.85, power = 3))
    ...
    for epoch in range(args.start_epoch, args.end_epochs):
        ...
        with open('../og/log_'+args.name+'.csv', 'a') as f:
            csv_writer = csv.writer(f)
