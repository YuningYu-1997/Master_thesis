# This script is to training the model with similarity cross-entropy loss
import os
...
# The functions to generate a target matrix
...
def train(train_loader, model, criterion, optimizer, num, accumulation_steps, use_gpu):
    ...
    for i, (images, target) in enumerate(pbar):
        ...
        label = target_matrix[target] # This matrix will be delivered to the loss function as the conpensation matrix
...
if __name__ == '__main__':
    ...
    #target_matrix = torch.from_numpy(generate_target_sim(wp_matrix, topk=5, power = 2))
