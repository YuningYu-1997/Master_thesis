...
def train(train_loader, model, criterion, optimizer, num, accumulation_steps, use_gpu):
    ...
    for i, (images, target) in enumerate(pbar):
        ...
        loss = criterion(output, df, target, label)
        losses += loss.item()
...
def validate(val_loader, model, criterion, num, use_gpu):
    ...
    with torch.no_grad():    
        for i, (images, target) in enumerate(tqdm.tqdm(val_loader, leave=False, position=0)):
            ...
            wp += dwp
            right_sum += train_correct.item()
            losses += criterion(output, df, target, label).item()
...
class customLoss(torch.nn.Module):
    ...
    def forward(self, output, df, target, label):
        probs = torch.nn.functional.softmax(output,dim=1)
        weighted_probs = torch.multiply(probs, label)
        sce = -torch.sum(torch.log(torch.sum(weighted_probs+self.eps,dim=1)))

        batch_size = label.shape[0] # batch size
        #real = torch.max(target,1)[1] #labels
        # modified cross entropy loss
        device_no = output.get_device()
        zeros = torch.zeros((batch_size,batch_size))
        eps = torch.tensor([[1e-9]]*batch_size)
        if device_no>-1:
        ...
        wp_Sim_3 = torch.pow(wp_Sim,3)
        #Flag = torch.eq(target.expand(batch_size,batch_size), target.expand(batch_size,batch_size).t()).long()
        #f = torch.where(Flag.long() == 0, -1, Flag.long())

        #Sim_sum = torch.sum(torch.maximum(torch.multiply(wp_Sim_3, torch.abs(torch.multiply(f, wp_Sim) - torch.multiply(f, cos_Sim))), zeros))
        Sim_sum = torch.sum(torch.multiply(wp_Sim_3, torch.abs(torch.pow(wp_Sim - cos_Sim,2))))
        return sce + self.lamda*Sim_sum
        #return CE
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name',default = 'resnet18_simcosloss_09^3_3*1^2_cusSet', type=str, help='Name of the saved file')
